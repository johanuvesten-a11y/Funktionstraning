...
# --- Ställ in import streamlit as st
import matplotlib
matplotlib.use('Agg') 
import matplotlib.pyplot as plt
import numpy as np
...
def generera_funktion():
    ...
    if typ == 'linjar':
        ...
        # En planare tredjegradskurva så den håller sig inom -10 till 10
        ...
        # Basen är 2 eller 0.5 så den träffar rutnätets hel/halvtal
        ...
        # Vi smyger in pi här för att lura vågen att vända exakt på hel- och halvtal
        a = random.randint(2, 4) * random.choice([-1, 1])
        period = random.choice([0.25, 0.5]) # Bestämmer hur "bred" vågen är
        h = random.randint(-2, 2)
    ...
    for _ in range(100): 
        ...
        if niva == 1:
            ...
            if fraga_typ == 'hitta_y':
                ...
                # Spärr: Fråga inte om x om det finns fler än 3 svar (t.ex. för sinusvågor)
            ...
            if fraga_typ == 'f_x_plus_c':
                ...
                if len(alla_mål_x) > 3: continue # Spärr för för många svarsrutor
                ...
                if len(alla_mål_x) > 3: continue # Spärr
...
with col_kontroller:
    ...
    if st.session_state.submitted_ans:
        if st.session_state.svar_status == 'ratt':
            ...
            st.warning("Fyll i alla rutor innan du rättar.") layout till bred ---
st.set_page_config(layout="wide", page_title="Läs av grafen")
...
def ny_uppgift():
    ...
    # Nollställ minnet för rättning när vi byter uppgift
    st.session_state.submitted_ans = False
    st.session_state.svar_status = None
...
with col_kontroller:
    ...
    # FIX: Spara resultatet i minnet innan omstarten
    if rattat:
        st.session_state.submitted_ans = True
    ...
    # FIX: Rita ut meddelandet baserat på minnet från st.session_state
    if st.session_state.submitted_ans:
        if st.session_state.svar_status == 'ratt':
            st.success("✅ Helt rätt! Snyggt jobbat.")
        elif st.session_state.svar_status == 'fel':
